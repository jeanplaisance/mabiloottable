...
addBonuses <- function(row, diff){
  
  bonus <- vector(mode="list", length=7)
  bonus[[1]] <- list(row$Boss.Pass, row$X)
  bonus[[2]] <- list(row$Elite.Pass, row$X)
  bonus[[3]] <- list(row$CostumedID, cosRates[j])
  bonus[[4]] <- list(row$EnhanceItem1, itemRates[j]/2)
  bonus[[5]] <- list(row$EnhanceItem2, itemRates[j]/2)
  bonus[[6]] <- list(row$StoneType, stoneRates[j])
  bonus[[7]] <- list(row$EnchantID, enchantRates[j])
...
createSMTables <- function(){
  ...
  for(i in 1:length(sm$Mission)){
    ...
    for(j in 1:5){
      df <- rbind(df, c("Name" = paste(sm$Mission[i], dict[[j]]), "Bonus" = addBonuses(sm[i,], j), missions[missions$ID==sm$Primary.Loot[i]&missions$Diff==j,3:8]))
    }
  }
